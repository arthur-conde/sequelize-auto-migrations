...
const parseIndex = function (idx) {
    ...
    if (idx.type)
        options.type = idx.type; // Set a type for the index, e.g. BTREE. See the documentation of the used dialect

    if (idx.indexType)
        options.indexType = idx.indexType;
    ...
    if (idx.using && idx.using != "")
        options.using = idx.using;

    if (idx.operator && idx.operator != "")
        options.operator = idx.operator;

    if (idx.where)
        options.where = idx.where;

    if (idx.concurrently && typeof idx.concurrently == "boolean")
        options.concurrently = idx.concurrently;
...
const getPartialMigration = function (actions) {
    let argsToStr = function (args) {
        for (let i = 0; i < args.length; i++) {
            const arg = args[i];
            for (let j in arg) {
                if (arg.hasOwnProperty(j))
                    return `Sequelize.${j}('${arg[j]}')`;
            }
        }
    }
    ...
    const sequelizeMethodToStr = function (field) {
        ...
        switch (fieldType) {
            ...
            case "Fn": {
                const fn = field.fn,
                    args = field.args.map(arg => isSequelizeMethod(arg) ? sequelizeMethodToStr(arg) : s(arg));
            ...
            case "Where": {
                const attribute = isSequelizeMethod(field.attribute) ? sequelizeMethodToStr(field.attribute) : s(field.attribute),
                    comparator = s(field.comparator),
                    logic = isSequelizeMethod(field.logic) ? sequelizeMethodToStr(field.logic) : s(field.attrlogicibute);
            ...
            case "Json": {
                if (field.conditions) {
                    const conditions = isSequelizeMethod(field.attribute) ? sequelizeMethodToStr(field.attribute) : s(field.attribute),
                        value = isSequelizeMethod(field.logic) ? sequelizeMethodToStr(field.logic) : s(field.attrlogicibute);
